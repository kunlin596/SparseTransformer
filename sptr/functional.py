...
class AttentionStep1(Function):
    ...
    def forward(ctx, q, k, index0, index0_offsets, index1, index1_offsets, n_max):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(f"attn_step1: forward time: {time.time() - t}s, M: {M}")
    ...
    def backward(ctx, grad_output):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(f"attn_step1: backward time: {time.time() - t}s, M: {M}")
...
class DotProdWithIdx(Function):
    ...
    def forward(
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(f"dot_prod_with_idx: forward time: {time.time() - t}s, M: {M}")
